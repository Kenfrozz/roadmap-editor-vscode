...
import { KuyrukGorevi, KokpitDurumBilgi, TamamlananGorev, RoadmapItem } from '../../types';
...
export class KokpitYonetici {
  ...
  private _webviewler: Set<vscode.Webview> = new Set();
  private _dinleyici: vscode.Disposable | null = null;
  ...
  registerWebview(w: vscode.Webview): void { this._webviewler.add(w); }
  unregisterWebview(w: vscode.Webview): void { this._webviewler.delete(w); }
  ...
  async baslat(kuyruk: KuyrukGorevi[]): Promise<void> {
    ...
    this._dinleyici?.dispose();
    this._dinleyici = vscode.window.onDidCloseTerminal((t) => {
      if (t !== this._terminal) return;
  ...
  async durdur(): Promise<void> {
    ...
    this._terminal = null; // dinleyicinin ignore etmesi icin once null yap
    if (term) term.dispose();
    this._dinleyici?.dispose();
    this._dinleyici = null;
    this._durum = 'bosta';
    this._kuyruk = [];
    this._aktifGorevIndex = -1;
    this.broadcastDurum();
  ...
  async dosyaDegisti(): Promise<void> {
    ...
    try {
      ...
      vscode.window.showInformationMessage(`Kairos: ${gorev.ozellik} — tamamlandi`);
      this.broadcast({
        command: 'kokpitGorevTamamlandi',
  ...
  private sonrakiGorev(): void {
    ...
    if (this._aktifGorevIndex >= this._kuyruk.length) {
      ...
      this._dinleyici = null;
      this.broadcastDurum();
      vscode.window.showInformationMessage('Kairos: Tum gorevler tamamlandi!');
      return;
    }
    this.broadcastDurum();
  ...
  private gorevCalistir(gorev: KuyrukGorevi): void {
    ...
    if (termOption) {
      termOpts.shellPath = termOption.path;
      if (termOption.args) termOpts.shellArgs = termOption.args;
    }

    this._terminal = vscode.window.createTerminal(termOpts);
    this._terminal.show();

    const safePrompt = prompt.replace(/"/g, '\\"');
    this._terminal.sendText(`claude --dangerously-skip-permissions "${safePrompt}"; exit $?`);
  }
