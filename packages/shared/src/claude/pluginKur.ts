...
function installViaTerminal(): void {
  ...
  terminalSaglayici.calistir({
    name: 'Kairos Plugin Kurulum',
    cmd: 'claude plugin marketplace remove kairos-marketplace 2>/dev/null; claude plugin marketplace add ./kairos && claude plugin install kairos@kairos-marketplace --scope project',
  });
}
