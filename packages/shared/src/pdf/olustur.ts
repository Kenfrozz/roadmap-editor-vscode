...
function drawCover(
  ...
  y += doc.heightOfString(upperName, { width: CW } as any) + 14;
...
function drawTableRow(
  ...
  for (let i = 0; i < columns.length; i++) {
    ...
    if (col.type === 'status') {
      const hex = STATUS_HEX[val as string] || P.subtle;
      const cx = x + w / 2;
      const cy = y + h / 2;
      const r = 7;
      ...
      doc.text(fmtDate(val as string), x + 10, y + 8, { width: w - 16, lineBreak: false, ellipsis: true });

    } else {
      const isFeature = col.key === 'ozellik';
      doc.font(isFeature ? 'Bold' : 'Regular').fontSize(8).fillColor(isFeature ? P.ink : P.body);
      doc.text(val as string, x + 10, y + 8, { width: w - 16, height: h - 10, lineBreak: false, ellipsis: true });
    }
