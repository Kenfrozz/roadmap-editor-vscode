...
import { cn, parsePrdRef, buildPrdRef } from '../lib/utils'
...
export function SortableRow({
  ...
  // PRD tiklandiginda hash dogrulamasi yap, duzeltme varsa referansi guncelle
  const handlePrdClick = () => {
    if (item.prd && onPrdRefUpdate) {
      const p = parsePrdRef(item.prd)
      if (p && p.hash) {
        api.prdLines(p.start, p.end, p.filename, p.hash).then(result => {
          if (result.duzeltildi) {
            onPrdRefUpdate(fazKey, item.id, buildPrdRef(p.filename, result.start, result.end, result.hash))
          }
        }).catch(() => {})
      }
    }
    onPrdClick(fazKey, item.id, item.prd)
  }
        ...
        >
          ...
          <div className="flex-1 min-w-0 px-1">
            <Input
              ...
              className={cn(
                'h-6 text-[10px] font-normal bg-transparent border-none px-1 text-foreground/80',
      ...
      >
        ...
        <div className="flex-1 min-w-0 px-1 py-0.5">
          <Input
            ...
            className={cn(
              'h-6 text-[11px] font-medium bg-transparent border-none px-1',
              'focus-visible:ring-1 focus-visible:ring-primary/30',
              allDone && 'line-through text-muted-foreground/50',
            )}
          />
