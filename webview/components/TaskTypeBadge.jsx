  ...
  return (
    <DropdownMenu modal={false}>
      <DropdownMenuTrigger asChild>
        {current ? (
          <button className={cn(
            'h-6 px-1.5 rounded-md flex items-center gap-1 text-[10px] font-semibold transition-colors',
            colorConfig.bg, colorConfig.text,
          )}>
            <Icon className="w-3 h-3 shrink-0" />
            <span className="hidden md:inline truncate max-w-[60px]">{current.label}</span>
          </button>
        ) : (
